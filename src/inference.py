...
import sys
sys.path.append("./python-libs/lib/python3.6/site-packages/")
...
def save_batch(filenames, predictions, outdir, suffix='predicted_mask.png'):
    for fn, pred in zip(filenames, predictions):
        ...
        if outdir is not None:
            ...
            im = Image.fromarray(pred.asnumpy())
            new_im=im.convert("L")            
            #print ("Saving ",os.path.join(outdir, mask_name))
            new_im.save(os.path.join(outdir, mask_name))
        else:
            #imsave(os.path.join(os.path.dirname(fn), mask_name), pred.asnumpy())
            im = Image.fromarray(pred.asnumpy())
            new_im=im.convert("L")
            print ("Saving ",os.path.join(os.path.dirname(fn), mask_name))
            new_im.save(os.path.join(os.path.dirname(fn), mask_name))          
...
def main():
    ...
    net = unet.Unet()
    print ("Loading", os.path.join(root, args.params))
